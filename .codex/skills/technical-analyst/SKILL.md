...
## Freedom_Multi_EN Boundary

Inside `/Users/newsong/Desktop/AIstudio/Freedom_Multi_EN`, this skill is an
auxiliary technical-evidence module. It may provide chart evidence to Buffett,
Poseidon, Hades, or a standalone chart diagnostic request, but it must not issue
formal project BUY/SELL/rebalance decisions or bypass the project-local
`buffett` skill for `buffett开始`.
...
This skill generates markdown analysis reports saved under the workflow task
folder or the project `report/` directory:
...
**File Naming Convention**: Save each analysis as `[SYMBOL]_technical_analysis_[YYYY-MM-DD].md` under the workflow task folder when one exists, otherwise under `report/`.
