...
description: |
  ...
  Integrates with the Skills CLI (npx skills find, npx skills add) to search, verify, and install project-level packages from the skills.sh directory.
  Trigger: "find a skill", "is there a skill for", "search skills", "install skill", "npx skills"
---
...
- `npx skills add <package>` — Install a skill from GitHub or other sources into the current project
- `npx skills check` — Check current-project skills for updates
- `npx skills update` — Update current-project skills

**Browse skills at:** https://skills.sh/

**Project policy:** install skills only at project scope under `.codex/skills/<skill-name>/SKILL.md`. Do not use global/user-level installs such as `npx skills add ... -g`, `~/.codex/skills`, or `$CODEX_HOME/skills` for this repository.
...
After installation, verify the skill exists as `.codex/skills/<skill-name>/SKILL.md`. If the CLI writes to a different local project path, normalize the result into `.codex/skills/` and keep any displaced files in `tmp/`.
