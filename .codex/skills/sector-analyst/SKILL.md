---
name: sector-analyst
description: Auxiliary sector rotation and market cycle analysis module. Buffett, Poseidon, or Zeus may call this skill for supporting sector evidence in Freedom_Multi_EN; do not bypass Buffett for formal sector-to-stock selection, portfolio action, current BUY/SELL decisions, or "buffett开始". Use directly only for standalone, non-decision sector diagnostics. All analysis and output are conducted in English unless a project workflow requests otherwise.
---
...
auxiliary sector evidence module. It can be called by Buffett, Poseidon, Zeus,
or a user for standalone sector diagnostics, but it must not replace the
Buffett full workflow for formal sector-to-stock selection, current portfolio
actions, rebalancing, or `buffett开始`.

If a request asks for final sector allocation, candidate tiers, current
BUY/SELL actions, or portfolio implementation in this project, defer to the
project-local `buffett` skill and provide only supporting sector evidence when
explicitly asked.
...
- User asks for standalone sector-based scenario analysis or predictions
- Buffett, Poseidon, or Zeus explicitly calls this skill as an auxiliary sector module
...
# Save to file
python3 scripts/analyze_sector_rotation.py --save --output-dir report/
```
...
5. **Diagnostic Positioning Evidence**: Strategic and tactical positioning evidence based on scenario probabilities; formal BUY/SELL/rebalance decisions remain with Buffett when a project decision is requested
...
## Diagnostic Positioning Evidence
