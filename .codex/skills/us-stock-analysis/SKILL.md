...
description: Auxiliary US stock analysis module for fundamentals, technicals, comparisons, and evidence reports. In Freedom_Multi_EN, Buffett may call this skill for supporting evidence; do not bypass Buffett for formal portfolio review, current BUY/SELL decisions, or the exact trigger "buffett开始". Use directly only for standalone, non-decision ticker analysis or when a project workflow explicitly calls it as an auxiliary module.
---
...
auxiliary evidence module. It can be called by Buffett, Poseidon, Zeus, or a
user for a narrow ticker brief, but it must not replace the Buffett full
workflow for portfolio review, position sizing, current BUY/SELL actions,
rebalancing, or `buffett开始`.

If a request asks for formal investment action in this project, defer to the
project-local `buffett` skill and provide only supporting stock-level evidence
...
**When to Use:** User asks for a detailed standalone report or complete ticker analysis.
For project portfolio action, use this only as supporting evidence for Buffett.
...
6. **Generate recommendation or evidence summary:**
   - Standalone non-decision analysis may include an evidence view or diagnostic
     view, but not a formal Buy/Hold/Sell decision.
   - Project portfolio decisions must defer final action to Buffett
...
6. **Generate recommendation or relative evidence summary:**
