...
auxiliary market-evidence module. It may support Buffett, Zeus, Poseidon, Hades,
or a standalone diagnostic market brief, but it must not bypass the project-local
`buffett` skill for formal portfolio review, sector-to-stock selection,
rebalancing, current BUY/SELL actions, or the exact trigger `buffett开始`.
...
4. Market evidence for Buffett or standalone positioning diagnostics
