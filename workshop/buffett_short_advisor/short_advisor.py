#!/usr/bin/env python3
"""Manual runner for Buffett short-term US trading advice."""
...
DEFAULT_CONFIG = {
    ...
    "system_log_path": "workshop/buffett_short_advisor/system_log.md",
}
...
def load_config(path: Path | None) -> AdvisorConfig:
    ...
    return AdvisorConfig(
        ...
        write_system_log=bool(data.get("write_system_log", data.get("append_operation_log", True))),
...
def ensure_base_short(path: Path) -> None:
    ...
    path.write_text(
        ...
        "- 建议系统不得自动修改“操作记录”；系统运行记录另存于 `workshop/buffett_short_advisor/system_log.md` 和 `runs/` 目录。\n"
        "- 所有建议仅作为投资研究记录，不是自动交易指令。\n\n"
        "## 操作记录\n\n"
        "<!--\n"
        "本节由用户手动填写真实交易记录。建议格式：\n\n"
...
def run_dir_for(timestamp: dt.datetime, cfg: AdvisorConfig) -> Path:
    ...
    return cfg.project_root / "workshop" / "buffett_short_advisor" / "runs" / stamp
...
运行时间：{local_ts}
主交易市场：{cfg.primary_market}，重点是美股短线交易建议。

硬性要求：
1. 先阅读并引用本次 base_short.md 快照中的持仓、剩余资金和用户手动填写的“操作记录”。
2. 在 Buffett 复盘阶段必须读取上一次同类 SQLite 决策；不得跳过历史决策读取。
3. `01_buffett_review.md` 必须同时复盘两类信息：用户手工交易记录的执行结果、上一条同类 SQLite 决策的成败/待验证状态。
...
8. 根据当前美股价格、盘中走势、重大新闻/财报/宏观催化、用户实际操作记录和上一次决策复盘，给出短线交易建议。
9. 建议必须区分：立即可执行、等待触发、止损/止盈、禁止追高、继续观察。
10. 不要自动下单。所有输出都是投资研究建议。
11. 不要改写 base_short.md 的“操作记录”；该区域只由用户手动填写真实交易。
12. 所有建议应优先保护已有仓位风险，尤其是美股持仓和港元可投资美股额度。
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_parser = sub.add_parser("run", aliases=["run-once"], help="Run one manual Buffett prompt.")
